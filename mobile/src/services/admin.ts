...
export async function resetStaffUserPassword(userId: string): Promise<boolean> {
  if (!isSupabaseConfigured) {
    await logActionMock('RESET_PASSWORD', 'profiles', userId, null, { remarks: 'Password reset successful to default credentials' });
    return true;
  }
...
export async function getReportData(dateRange: { start: string; end: string }): Promise<ClinicReportData> {
  ...
  if (!isSupabaseConfigured) {
    ...
    // 7. Treatment Category Breakdown (Hair, Face/Skin, Both)
    const treatments = await getLocalData<any[]>('medflowx_treatments', []);
    const rangeTreatments = treatments.filter((t) => {
    ...
    rangeTreatments.forEach((t) => {
      const amt = Number(t.treatment_amount) || 0;
      if (t.diagnosis_type === 'Hair Diagnosis') {
        hairRevenue += amt;
        hairCount += 1;
      } else if (t.diagnosis_type === 'Skin Diagnosis') {
        skinRevenue += amt;
        skinCount += 1;
      } else {
        bothRevenue += amt;
    ...
    const treatmentCategory = {
      ...
      skinRevenue,
      bothRevenue,
      totalTreatmentRevenue: hairRevenue + skinRevenue + bothRevenue,
      hairCount,
      skinCount,
      bothCount,
      totalTreatmentCount: hairCount + skinCount + bothCount,
    };
  ...
  const [paymentsData, patientsData, visitsData, doctorStatsData, treatmentsData] = await Promise.all([
    ...
    supabase.from('treatments').select('diagnosis_type, treatment_amount, created_at').gte('created_at', startIso).lte('created_at', endIso)
  ]);
  ...
  // Treatments Category breakdown
  const treatmentsList = treatmentsData.data || [];
  let hairRev = 0;
  let skinRev = 0;
  let bothRev = 0;
  let hairC = 0;
  ...
  const treatmentCategory = {
    ...
    skinRevenue: skinRev,
    bothRevenue: bothRev,
    totalTreatmentRevenue: hairRev + skinRev + bothRev,
    hairCount: hairC,
    skinCount: skinC,
    bothCount: bothC,
    totalTreatmentCount: hairC + skinC + bothC,
  };
